  ...
  $("agentsTable").innerHTML = state.agents.filter((a) => a.active && !agentStatus(a).ended).map((a) => {
    ...
    return `
      <tr>
        ...
        <td>${a.business}</td>
        <td>${a.work_start} - ${a.work_end}</td>
        <td>${a.break_start || "-"} - ${a.break_end || "-"}</td>
...
function bindEvents() {
  ...
  $("ticketBusiness").addEventListener("change", renderAssignees);
  $("queueBusiness").addEventListener("change", (e) => {
    state.queueBusiness = e.target.value;
    renderQueue();
  });
...
async function handleDocumentClick(event) {
  ...
  if (editId) {
    ...
    $("agentStart").value = a.work_start;
    $("agentBreakStart").value = a.break_start || "";
    $("agentBreakEnd").value = a.break_end || "";
    $("agentEnd").value = a.work_end;
    showView("peopleView");
  }
